  ...
  ui <- fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        textInput("category", "Category", value = "category 'workflow automation'"),
        textInput("package", "Package", value = "coolr"),
        textInput("package_does", "Package Does", value = "does cool things in R"),
        textInput("package_check_shows", "Package Check Shows", value = "this package is in top shape"),
        textInput("in_scope_because", "In Scope Because", value = "it streamlines necessary processes"),
        textInput("my_checks_show", "My Checks Show", value = "the package is awesome"),
        textInput("my_checks_url", "My Checks URL", value = "https://github.com/ropensci/software-review/issues/000#issuecomment-0000000000"),
        textInput("category_description", "Category Description", value = "Tools that automate and link together workflows, such as build systems and tools to manage continuous integration. Does not include general tools for literate programming. (e.g., R markdown extensions not under the previous topics). (Example: drake)")
      ),
      mainPanel(
